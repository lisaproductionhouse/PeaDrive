...
pub enum WorkerEvent {
    ...
    /// Kiểm tra chủ sở hữu VÀ xóa/chuyển đi được gộp làm 1 giai đoạn liên
    /// tục cho từng file (không tách "kiểm tra hết rồi mới hỏi rồi mới
    /// làm" như trước) — `skipped` đếm số file KHÔNG do mình sở hữu mà
    /// người dùng không tick "cho phép chuyển vào thư mục tạm", nên bị bỏ
    /// qua hoàn toàn (không đụng tới).
    BulkTrashProgress {
        done: usize,
        total: usize,
    },
