...
8. **Tải theo danh sách tên** (không cần đăng nhập Google): mở mục **"Tải /
   xóa hàng loạt theo danh sách tên"** bên dưới danh sách file → dán tên các
   file (hoặc thư mục) cần tải, hoặc **kéo thả 1 file `.txt`** chứa danh sách
   vào cửa sổ app (tự nối vào ô nhập) → bấm **Tìm & xem trước** → app liệt kê
   các mục khớp (kèm dung lượng) và riêng các tên **không khớp** để bạn kiểm
   tra lại → bấm **⬇ Tải N mục**. Thư mục lưu, chính sách trùng tên, tiến độ
   và nút Hủy dùng y như các cách tải ở trên. Định dạng danh sách (dấu phân
   cách, tên có khoảng trắng...) xem ở Mục 6 — dùng chung với xóa hàng loạt.
   - Tên phải khớp **CHÍNH XÁC** (phân biệt hoa/thường) với tên đang hiển
     thị trong app; file Google Docs/Sheets/Slides hiển thị không có đuôi
     (`.docx`/`.xlsx`/`.pptx`) nên trong danh sách cũng không ghi đuôi.
   - Chỉ khớp với các mục nằm **trực tiếp trong thư mục đang mở**, không tìm
     đệ quy vào thư mục con. Danh sách đã nhập được giữ nguyên khi chuyển
     thư mục, nên nếu các file nằm rải rác nhiều nơi: mở lần lượt từng thư
     mục rồi bấm **Tìm & xem trước** + **Tải** lại.
   - Nếu 1 tên trùng với tên 1 **thư mục**, cả thư mục đó (đệ quy) sẽ được
     tải. Nếu Drive có nhiều file cùng tên trong thư mục, tất cả đều được tải
     (tên trùng trên ổ đĩa tự thêm `(2)`, `(3)`...).
...
- **Xóa 1 file**: bấm 🗑 trên dòng đó — app tự kiểm tra trước xem tài khoản
  đang đăng nhập có phải **chủ sở hữu** file đó không (Google chỉ cho phép
  CHỦ SỞ HỮU chuyển thẳng vào Thùng rác, quyền "Người chỉnh sửa" qua link
  KHÔNG đủ — xem mục Giới hạn bên dưới), rồi hiện đúng banner xác nhận:
  - Là chủ sở hữu → banner quen thuộc "Chuyển vào Thùng rác?" → bấm **Xóa**.
    App **không xóa vĩnh viễn** — chỉ chuyển vào Thùng rác của Drive, có thể
    khôi phục lại trong 30 ngày nếu lỡ tay.
  - KHÔNG phải chủ sở hữu → banner hiện rõ **tên/email chủ sở hữu thật sự**
    kèm lựa chọn **Đồng ý, chuyển đi**: file sẽ được **CHUYỂN** (không xóa)
    sang 1 thư mục con tên `_Đã đánh dấu xóa (chờ chủ sở hữu xóa hẳn)` ngay
    trong thư mục hiện tại, để chủ sở hữu tự ghé qua dọn/xóa hẳn sau — đây
    là cách duy nhất "dọn" được file khi mình không phải chủ sở hữu, vì
    Google không cho phép chuyển thẳng vào Thùng rác trong trường hợp này.
- **Xóa hàng loạt theo danh sách tên**: mở mục "Tải / xóa hàng loạt theo
  danh sách tên" (cùng ô nhập với tải theo danh sách ở Mục 5, nút xóa chỉ hiện
  khi đã đăng nhập) → dán/nhập tên các file cần xóa, hoặc **kéo
  ...
  danh sách khớp được liệt kê ra → bấm **Chuyển N file vào Thùng rác** → app
  kiểm tra chủ sở hữu từng file rồi hiện bảng tổng hợp ("X file bạn sở hữu
  sẽ xóa thẳng, Y file không phải sẽ chuyển sang thư mục riêng, chủ sở hữu:
  ...") → bấm **Đồng ý, tiếp tục** để thực hiện thật.
...
- **Chỉ CHỦ SỞ HỮU mới chuyển thẳng file vào Thùng rác được** — đây là quy
  định cứng của Google (developers.google.com/workspace/drive/api/guides/delete),
  áp dụng cho MỌI ứng dụng kể cả giao diện Google Drive gốc: quyền "Người
  chỉnh sửa" (kể cả qua "Bất kỳ ai có đường liên kết") cho phép sửa nội
  dung, đổi tên... nhưng KHÔNG đủ để xóa nếu không phải chủ sở hữu. App tự
  phát hiện trường hợp này và đề xuất phương án thay thế: chuyển file sang
  thư mục `_Đã đánh dấu xóa (chờ chủ sở hữu xóa hẳn)` để chủ sở hữu tự dọn
  (xem Mục 6) — **đổi tên thì không bị giới hạn này**, quyền Editor bình
  thường vẫn đổi tên được.
