...
impl DriveClient {
        ...
        loop {
            ...
            match self.http.get(url.clone()).send().await {
                Ok(resp) => {
                    ...
                    let message = match parse_api_error_message(&body) {
                        Some(m) => format!("HTTP {status_code}: {m}"),
                        None => format!("HTTP {status_code}"),
                    };
                    let retryable =
                        status_code == 403 || status_code == 429 || status.is_server_error();
    ...
    pub async fn get_metadata(&self, file_id: &str) -> anyhow::Result<DriveEntry> {
        ...
        let resp = self.get_with_retry(url).await.map_err(|e| {
            anyhow::anyhow!(
                "Không truy cập được thư mục/tệp này. Hãy kiểm tra: (1) link đã đúng, \
                 (2) đã chia sẻ ở chế độ \"Bất kỳ ai có đường liên kết\", (3) API key hợp lệ. \
                 Chi tiết: {e}"
            )
        })?;
        let text = resp.text().await?;
...
impl DriveEditClient {
    ...
    pub async fn rename_file(
        ...
        let resp = self
            ...
            .await
            .map_err(|e| anyhow::anyhow!("Lỗi kết nối mạng: {e}"))?;
        check_write_response(resp).await
    }
...
async fn check_write_response(resp: reqwest::Response) -> anyhow::Result<()> {
    let status = resp.status();
    if status.is_success() {
        return Ok(());
    }
