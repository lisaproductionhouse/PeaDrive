...
enum ConfirmDeleteState {
    ...
    Ready {
        entry: DriveEntry,
        owned_by_me: bool,
        /// Tên/email chủ sở hữu — chỉ có giá trị khi `owned_by_me` là
        /// `false`, để hiện lên banner.
...
pub struct GDriveCopierApp {
    ...
    bulk_delete_checked: HashSet<String>,
...
impl GDriveCopierApp {
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        Self {
            ...
            bulk_delete_checked: HashSet::new(),
            runtime,
            event_tx,
            event_rx,
        }
        ...
        while let Ok(event) = self.event_rx.try_recv() {
            match event {
                ...
                WorkerEvent::OwnershipChecked { entry, info } => {
                    self.confirm_delete = Some(ConfirmDeleteState::Ready {
                        entry,
                ...
                WorkerEvent::BulkOwnershipChecked(plans) => {
                    self.bulk_delete_checked = plans.iter().map(|p| p.entry.id.clone()).collect();
            ...
            match state {
                ...
                ConfirmDeleteState::Ready {
                    ...
                    ui.colored_label(
                        ...
                        format!(
                            "Bạn không phải chủ sở hữu file '{}' — chủ sở hữu là: {owner}. \
                             Google chỉ cho phép chủ sở hữu chuyển thẳng vào Thùng rác (quyền \
                             Editor không đủ). Nếu tiếp tục, file này sẽ được CHUYỂN sang thư \
                             mục \"{QUARANTINE_FOLDER_NAME}\" ngay trong thư mục hiện tại — \
                             KHÔNG bị xóa — để {owner} tự vào dọn/xóa hẳn sau.",
                            entry.name
                        ),
                    );
                    ui.horizontal(|ui| {
                        let can_proceed = parent_id.is_some();
                        if ui
                            .add_enabled(can_proceed, egui::Button::new("Đồng ý, chuyển đi"))
                            .clicked()
                        {
                            self.confirm_delete = None;
    ...
    fn draw_bulk_delete_section(&mut self, ui: &mut egui::Ui) {
        ...
        let checked_snapshot = self.bulk_delete_checked.clone();
                ...
                if !matches_snapshot.is_empty() {
                    ...
                    match &phase_snapshot {
                        ...
                        BulkDeletePhase::ReadyToConfirm(plans) => {
                            ...
                            ui.weak(format!(
                                "{owned_count} file do bạn sở hữu (→ Thùng rác), \
                                 {not_owned_count} file không phải (→ chuyển sang thư mục \
                                 riêng \"{QUARANTINE_FOLDER_NAME}\"). Đã tick sẵn hết — bỏ tick \
                                 mục nào không muốn xử lý:"
                            ));
                            ui.horizontal(|ui| {
                                if ui.small_button("Tick tất cả").clicked() {
                                    toggle_all = Some(true);
                                }
                                if ui.small_button("Bỏ tick tất cả").clicked() {
                                    toggle_all = Some(false);
                                }
                            });
                            egui::ScrollArea::vertical().max_height(160.0).show(ui, |ui| {
                                for plan in plans {
                                    let mut checked = checked_snapshot.contains(&plan.entry.id);
                                    ui.horizontal(|ui| {
                                        if ui.checkbox(&mut checked, "").changed() {
                                            toggles.push((plan.entry.id.clone(), checked));
                                        }
                                        ui.label(&plan.entry.name);
                                        if plan.owned_by_me {
                                            ui.weak("→ Thùng rác");
                                        } else {
                                            let owner =
                                                plan.owner_label.as_deref().unwrap_or("không rõ");
                                            ui.colored_label(
                                                egui::Color32::from_rgb(224, 176, 96),
                                                format!("→ chuyển đi (chủ sở hữu: {owner})"),
                                            );
                                        }
                                    });
                                }
                            ...
                            ui.horizontal(|ui| {
                                let label = format!("Đồng ý, xử lý {checked_count} mục đã tick");
                                if ui
                                    .add_enabled(
                                        checked_count > 0,
        ...
        if let Some(all_checked) = toggle_all {
            if let BulkDeletePhase::ReadyToConfirm(plans) = &phase_snapshot {
                if all_checked {
                    self.bulk_delete_checked = plans.iter().map(|p| p.entry.id.clone()).collect();
                } else {
                    self.bulk_delete_checked.clear();
                }
            }
        }
        for (id, checked) in toggles {
            if checked {
                self.bulk_delete_checked.insert(id);
            } else {
                self.bulk_delete_checked.remove(&id);
            }
        }
        if find_clicked {
        ...
        if execute_clicked {
            if let BulkDeletePhase::ReadyToConfirm(plans) = phase_snapshot {
                let selected: Vec<downloader::BulkDeletePlanItem> = plans
                    .into_iter()
                    .filter(|p| self.bulk_delete_checked.contains(&p.entry.id))
                    .collect();
                self.bulk_delete_phase = BulkDeletePhase::Idle;
                self.bulk_delete_checked.clear();
...
impl eframe::App for GDriveCopierApp {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ui, |ui| {
            ...
            // nhận, tiến độ, nhật ký...) đặt trong 1 vùng CUỘN ĐƯỢC, chiếm
            // hết chiều cao còn lại của cửa sổ — trước đây xếp thẳng lên
            // trang nên khi tổng nội dung cao hơn cửa sổ, các nút ở cuối
            // (đặc biệt banner xác nhận xóa) có thể bị đẩy ra ngoài vùng
            // nhìn thấy mà không cách nào cuộn tới được.
