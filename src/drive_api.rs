...
/// Dựng 1 `reqwest::Client` DÙNG CHUNG cho MỌI lời gọi Google Drive API
/// trong suốt vòng đời app (cả đọc bằng API key ở `DriveClient` lẫn ghi
/// bằng OAuth ở `DriveEditClient`, và cả các lời gọi tới `oauth.rs`) —
/// `reqwest::Client` giữ pool kết nối keep-alive + cấu hình TLS bên trong,
/// nên dựng lại nhiều lần (mỗi lần mở thư mục / mỗi thao tác đổi tên-xóa
/// đều tự tạo client riêng như trước đây) vừa tốn công nạp lại root CA,
/// vừa mất khả năng tái dùng kết nối TCP/TLS đã mở tới googleapis.com giữa
/// các lần gọi liên tiếp. `reqwest::Client` rất rẻ để `.clone()` (chỉ tăng
/// refcount Arc bên trong), nên chỉ cần dựng đúng 1 lần ở đây.
...
impl DriveClient {
    /// Nhận vào 1 `reqwest::Client` đã dựng sẵn (xem `build_shared_http_client`)
    /// thay vì tự dựng riêng — dùng chung pool kết nối với `DriveEditClient`.
...
impl DriveEditClient {
    /// Nhận vào CÙNG 1 `reqwest::Client` dùng chung với `DriveClient` (xem
    /// `build_shared_http_client`) — không tự dựng client riêng nữa.
    ...
    pub async fn check_ownership(
        ...
        let resp = self
            ...
            .bearer_auth(access_token)
            .query(&[
                ("fields", "owners(displayName,emailAddress,me),parents"),
    ...
    pub async fn move_file(
        ...
        let resp = req
            ...
            .await
            .map_err(|e| anyhow::anyhow!("Lỗi kết nối mạng: {e}"))?;
        check_write_response(resp, WriteOperation::Move)
            .await
            .map_err(|e| {
                // Kèm sẵn parent cũ/mới đã gửi lên vào thông báo lỗi — nếu
                // vẫn gặp lỗi liên quan tới "parents", nhìn thấy ngay 2 ID
                // này để đối chiếu trực tiếp với "Vị trí" của file đó trên
                // giao diện Google Drive, thay vì phải đoán.
                anyhow::anyhow!(
                    "{e} (from_parent_id={from_parent_id}, to_parent_id={to_parent_id})"
                )
            })
    }
}
