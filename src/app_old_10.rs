...
impl GDriveCopierApp {
    ...
    fn drain_events(&mut self) {
        while let Ok(event) = self.event_rx.try_recv() {
            match event {
                ...
                WorkerEvent::BulkOwnershipChecked(mut plans) => {
                    // Cùng lý do như nhánh OwnershipChecked ở trên — dùng
                    // thư mục đang mở làm thư mục cha cho MỌI file không
                    // sở hữu, không tin vào field 'parents' Google trả về
                    // lúc kiểm tra sở hữu (đã thấy bị rỗng cho ít nhất 1
                    // file dù hoàn toàn bình thường).
                    let current_folder_id = self.breadcrumbs.last().map(|(id, _)| id.clone());
                    for plan in &mut plans {
                        if !plan.owned_by_me {
                            plan.parent_id = current_folder_id.clone();
                        }
                    }
    ...
    fn start_check_bulk_ownership(&mut self, entries: Vec<DriveEntry>) {
        ...
        let tx = self.event_tx.clone();
        let http = self.http.clone();
        // Giới hạn số request kiểm tra sở hữu chạy đồng thời — dùng lại
        // đúng số luồng tải song song đã cấu hình, để không bắn quá nhiều
        // request cùng lúc lên Google (đỡ làm nặng thêm rủi ro rate-limit
        // 429 đã biết), thay vì kiểm tra tuần tự từng file một như trước.
        ...
        self.runtime.spawn(async move {
            ...
            // Kiểm tra sở hữu SONG SONG có giới hạn (trước đây tuần tự
            // từng file, N file = N round-trip nối tiếp nhau) —
            // `buffer_unordered` chạy nhiều future cùng lúc TRONG CHÍNH
            // tác vụ nền này (không spawn task OS riêng cho mỗi file nên
            // không cần Arc/clone `edit_client`/`access_token`).
            let mut results: Vec<(usize, Result<downloader::BulkDeletePlanItem, String>)> =
                stream::iter(entries.into_iter().enumerate())
                    .map(|(idx, entry)| {
                        let edit_client = &edit_client;
                        let access_token = &access_token;
                        async move {
                            let outcome = match edit_client
                                .check_ownership(access_token, &entry.id)
                                .await
                            {
                                Ok(info) => Ok(downloader::BulkDeletePlanItem {
                                    entry,
                    ...
                    .collect()
                    .await;
            // buffer_unordered trả kết quả theo thứ tự HOÀN THÀNH, không
            // theo thứ tự ban đầu — sắp lại theo idx để hành vi & thông
            // báo lỗi (nếu có) nhất quán, giống hệt bản tuần tự cũ.
            results.sort_by_key(|(idx, _)| *idx);

            // Giữ đúng hành vi cũ: hễ có 1 file kiểm tra lỗi là báo lỗi đó
            // (theo thứ tự ban đầu) và dừng, không gửi 'BulkOwnershipChecked'.
            let mut plans = Vec::with_capacity(results.len());
            for (_, outcome) in results {
                match outcome {
                    Ok(plan) => plans.push(plan),
                    Err(msg) => {
                        let _ = tx.send(WorkerEvent::BulkOwnershipCheckError(msg));
                        return;
                    }
                }
            }
    ...
    fn start_bulk_delete_execute(&mut self, plans: Vec<downloader::BulkDeletePlanItem>) {
        ...
        let tx = self.event_tx.clone();
        let http = self.http.clone();
        // Giới hạn đồng thời giống hệt lý do ở start_check_bulk_ownership —
        // trước đây xóa/chuyển từng file một, N file = N round-trip nối tiếp.
        let concurrency = self.config.max_concurrent_downloads.max(1);
        ...
        self.runtime.spawn(async move {
            ...
            // Mỗi file được xử lý ĐỘC LẬP với nhau (không như kiểm tra sở
            // hữu ở trên — 1 file lỗi không ảnh hưởng các file khác), nên
            // chạy song song có giới hạn ở đây đơn giản hơn: không cần sắp
            // lại theo thứ tự ban đầu, cứ báo tiến độ/kết quả ngay khi
            // từng file xong (thứ tự dòng log có thể xen kẽ khác bản tuần
            // tự cũ 1 chút, nhưng số liệu cuối cùng giống hệt).
            let mut result_stream = stream::iter(plans)
                .map(|plan| {
                    let edit_client = &edit_client;
                    let access_token = &access_token;
                    async move {
