                    ...
                    match &phase_snapshot {
                        ...
                        BulkDeletePhase::ReadyToConfirm(plans) => {
                            ...
                            if not_owned_count == 0 {
                                ui.label(format!(
                                    "Cả {owned_count} file đều do bạn sở hữu — sẽ chuyển thẳng \
                                     vào Thùng rác."
                                ));
                            } else {
                                ...
                                ui.colored_label(
                                    egui::Color32::from_rgb(224, 176, 96),
                                    format!(
                                        "{owned_count} file do bạn sở hữu — sẽ chuyển thẳng vào \
                                         Thùng rác. {not_owned_count} file KHÔNG do bạn sở hữu \
                                         (chủ sở hữu: {}) — Google không cho xóa thẳng (chỉ chủ \
                                         sở hữu mới được) nên sẽ CHUYỂN sang thư mục \
                                         \"{QUARANTINE_FOLDER_NAME}\" thay vì xóa, để chủ sở \
                                         hữu tự dọn sau.",
                                        owners.join(", ")
                                    ),
                                );
                            ...
                            ui.horizontal(|ui| {
                                if ui.button("Đồng ý, tiếp tục").clicked() {
